...
@np.vectorize
def response(x, par0, par1, par2, par3, par4, par5, par6):

    t = x-par0;
    A0 = par1;
    tp = par2;
    CT = 1./1.996;
    A = A0 * 2.7433/pow(tp*CT,4);
    p0 = 1.477/tp/CT;
    pr1 = 1.417/tp/CT;
    pr2 = 1.204/tp/CT;
    pi1 = 0.598/tp/CT;
    ...
    #  print(t, A, tp, p0, pr1, pr2, pi1, pi2, k3, k4, k5, k6)
    #  print("---")
    #  print(
    ...
    #  )
    #  print(
    #          (exp(k4*t)*(k4 - k6)*(k4 - p0)*(pow(k4,2) + pow(pi1,2) - 2*k4*pr1 + pow(pr1,2))*(pow(k4,2) + pow(pi2,2) - 2*k4*pr2 + pow(pr2,2)))
    #  )
    #  print(
    #          (exp(k6*t)*(k4 - k6)*(k6 - p0)*(pow(k6,2) + pow(pi1,2) - 2*k6*pr1 + pow(pr1,2))*(pow(k6,2) + pow(pi2,2) - 2*k6*pr2 + pow(pr2,2)))
    #  )
    #  print(
    #          (exp(p0*t)*(k4 - p0)*(-k6 + p0)*(pow(p0,2) + pow(pi1,2) - 2*p0*pr1 + pow(pr1,2))*(pow(p0,2) + pow(pi2,2) - 2*p0*pr2 + pow(pr2,2)))
    #  )
...
pars = [4, 47.*1.012*100.*1.07, 2.2, 0.1, 0.1, 0.03, 0.03]
p_bounds = [20, 100000, 5., 10, 10, 2., 2.]

f = h5py.File('test.h5', 'r')
wfs = f['CE0/DelayRun/Waveform'][()]
...
def plot_chan(chn):
    ...
    wf = wfs[chn][::32]
    ppos = np.where(wf == np.max(wf))[0][0]
    wf = wf[ppos-10:ppos+50]
    xx = np.array(range(maxtick), dtype=np.float64)/factor

    popt, pcov = curve_fit(response, xdata=xx, ydata=wf, p0=pars, bounds=(0, p_bounds), maxfev=5000)
    print(popt, pcov)
    ...
    ax1.plot(xx, wf, marker='.')
    xx2 = np.arange(0., maxtick/factor, 0.05)
    ax1.plot(xx2, response(xx2, *popt), 'r-')
    textstr = '\n'.join(
                    ['par%d : %.2f'%(i, popt[i]) for i in range(len(popt))]
                )
    ax1.text(0.7, 0.4, textstr, transform=ax1.transAxes)
    ax1.set_title('Channel : %02d'% chn)

    ax2.plot(xx, wf, marker='.')
    xx2 = np.arange(0., maxtick/factor, 0.05)
    ax2.plot(xx2, response(xx2, *popt), 'r-')
    ax2.set_ylim(-50, 100)

    plt.savefig('wf_fit/chn%02d.png'%chn)
